...
//    HAL_GPIO_WritePin(S4_E_PORT, S4_E_PIN, GPIO_PIN_RESET);
			HAL_GPIO_WritePin(HC4514_ENABLE_PORT, HC4514_ENABLE_PIN, GPIO_PIN_RESET);
}
...
static inline void X_Set_ByStep(uint8_t s)
{
    ...
    */
    ...
    uint8_t offset = (uint8_t)((3u - s) & 0x03u);  // (3 - s) mod 4

    for (uint8_t i = 0; i < 4; ++i)
    {
        uint8_t val = (uint8_t)((i + offset) & 0x03u); // 0..3 => Y0..Y3
        DecoderGroup_t xg = (DecoderGroup_t)(DECODER_X1 + i);
        Decoder_Select_2to4(xg, val);
    }
}
...
//    GPIO_Output_Init(S2_PORT, S2_PIN);
    ...
    // S1..S4 LE/E
		GPIO_Output_Init(HC4514_ENABLE_PORT, HC4514_ENABLE_PIN);
